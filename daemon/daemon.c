/*
 *以前我们在看《unix环境高级编程》的时候，有专门的整章详细介绍如何编写一个后台daemon程序（精灵程序），主要涉及到创建会话组，
 *切换工 作目录，设置文件屏蔽字，关闭不必要的描述符等多个操作。这些操作对于每一个后台程序来说都是类似的。
 ...
 *并且能够看到，进程的当前工作目录(cwd)为根目录/，daemon函数已经帮我们完成了daemon化的过程，接下来我们只需要关注于程序功能
 *的实现了。
 */
